...
#List of SQL files
sql_files = ['db/queries0.sql','db/queries1.sql', 'db/queries2.sql', 'db/queries3.sql', 'db/queries4a.sql','db/queries4b.sql', 'db/queries5a.sql','db/queries5b.sql', 'db/queries6.sql','db/queries7.sql']  
...
# Read .sql query and print comments in it 
    ...
    with open(sql_file_name, 'r') as sql_file:
        for line in sql_file:
            sql_queries += line
            if line.strip().startswith('-- VISUALIZATION image is opening in a new window'):
